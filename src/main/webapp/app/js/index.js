...
ProductApp.category.list = function() {
    $.get("./api/category", function(data) {
        ...
        if ((_embedded = data["_embedded"]) != null
            ...
            var tbody = $("#categories_table > tbody");
            // empty list
            if (category.length == 0) {
                tbody.append($("<tr>")
                        .append($("<td>").attr("colspan", "3").text("No categories."))
                        );
            }
            ...
            $.each(category, function(ix, c) {
                ...
                tbody.append($("<tr>")
                        ...
                        .append($("<td>")
                            .append($("<button>").attr({
                                "id": "category_edit_" + id,
                                "class": "category_edit btn"
                            }).text("Edit")).append(" ")
                            .append($("<button>").attr({
                                "id": "category_delete_" + id,
                                "class": "category_delete btn btn-danger"
                            }).text("Delete"))
                            )
                        );
                
                product_category.append(
                        $("<option>").attr("value", id).text(c["name"])
                        );
            });
